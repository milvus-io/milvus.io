...
async function execute() {
    ...
    files.forEach(file => {
      ...
      if (normalizedFile.match(/\.js$/)) {
        ...
        console.log('========= handle ', normalizedFile)
        if (langParts.indexOf('en') !== -1) {
          ...
          for (let i = 0; i < enabledLanguages.length; i++) {
            ...
            if (
              ...
              console.log(targetFile.replace(`${sep}en${sep}`, sep + language + sep));
            }
          }
          ...
          console.log('base', targetFile.replace(`${sep}en${sep}`, sep))
          ...
          console.log('else ', targetFile.replace(`${sep}en${sep}`, sep), language, langParts)
        ...
        mkdirp.sync(path.dirname(targetFile));
        fs.copySync(normalizedFile, targetFile);
        console.log('copy other non .js files', targetFile)
      }
    });
